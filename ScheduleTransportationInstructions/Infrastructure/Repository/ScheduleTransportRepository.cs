...
namespace Infrastructure.Repository
{
    public class ScheduleTransportRepository : IScheduleTransportRepository
    {
        ...
        public async Task<ScheduleTransport> InstructionExists(int instructionId,int productId)
        {
            try
            {
                var dataExist = await _applicationDbContext.ScheduleTransport.Where(x => x.InstructionId == instructionId && x.ProductId == productId).FirstOrDefaultAsync();
                return dataExist;
            }
            catch (Exception ex)
            {
